export default class {
    constructor(table) {
        ...
        this.addEditActionsEvents();
        document.body.addEventListener('eventTableRenderData', () => this.addEditActionsEvents());
    }
    dotsHandler(event) {
        debugger;
    ...
    copyHandler(event) {
        ...
        this.add(newObject, this.table.data);
    ...
    editHandler(event) {
        ...
        for(let i = 1; i < currentTd.length-1; i++ ){
            ...
            if(isName) {
                ...
                let sortedCateg = this.table.data.map((elem) => elem.category).sort();
